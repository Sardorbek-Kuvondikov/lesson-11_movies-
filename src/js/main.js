...
const moviesRender = (array, node, regex = "") => {
  ...
  array.forEach((movie) => {
    ...
    if (regex !== "(?:)" && regex) {
      const titleElement = moviesClone.querySelector(".js-movies-title");
      const titleText = movie.title.toString();
      const markRegex = new RegExp(`(${regex.source})`, "gi");
      const titleParts = titleText.split(markRegex);
      titleElement.innerHTML = "";
      titleParts.forEach((part) => {
        const span = document.createElement("span");
        if (part.match(markRegex)) {
          span.innerHTML = `<mark class="bg-red-500 text-white">${part}</mark>`;
        } else {
    ...
    moviesClone.querySelector(".js-movies-categories").textContent =
      movie.categories.toString().replaceAll(",", ", ");
    moviesClone.querySelector(".js-modal-btn").dataset.imdbId = movie.imdb_id;
    moviesClone.querySelector(".js-convas-btn").dataset.convasImdbId =
      movie.imdb_id;
...
const elConvasTitle = document.querySelector(".js-convas-title");
const elConvasCategories = document.querySelector(".js-convas-categories");
const elConvasIframe = document.querySelector(".js-convas-video");
const elConvasRunting = document.querySelector(".js-convas-rating");
const elConvasYear = document.querySelector(".js-convas-year");
const elConvasClock = document.querySelector(".js-convas-hour");
const elConvasSummary = document.querySelector(".js-convas-summary");

const moviesConvasRender = (movie) => {
  elConvasTitle.textContent = movie.title;
  elConvasCategories.textContent = String(movie.categories.join(", "));
  elConvasIframe.src = movie.iframe_link;
  elConvasRunting.textContent = movie.imdb_rating;
  elConvasYear.textContent = movie.movie_year;
  elConvasClock.textContent = runtimeToMin(movie.runtime);
  elConvasSummary.textContent = movie.summary;
};
...
// Book Mark
const elConvas = document.querySelector(".js-convas");
const elConvasBtn = document.querySelector(".js-convas-btn");

let array = [];
elMoviesReult.addEventListener("click", (evt) => {
  evt.preventDefault();
  if (evt.target.matches(".js-convas-btn")) {
    const convasImdbId = evt.target.dataset.convasImdbId;
    evt.target.textContent = "Bookmark";
    movies.find((item) => {
      if (item.imdb_id === convasImdbId) {
        array.push(item);
        moviesConvasRender(item);
      }
    });
    console.log(array);

    elConvas.classList.remove("hidden");
  }
});

// Convas none
const elConvasNone = document.querySelector(".js-convas-none");
elConvasNone.addEventListener("click", (evt) => {
  evt.preventDefault();
  elConvas.classList.add("hidden");
});
